...
interface CookiesClientProps {
  initialPolicy: PolicyData | null
  initialLang: string
}

export default function CookiesClient({ initialPolicy, initialLang }: CookiesClientProps) {
  ...
  useEffect(() => {
    if (lang !== initialLang) {
      const fetchPolicy = async () => {
        try {
          const res = await fetch(`/api/policies?type=COOKIES&lang=${lang}`)
          if (res.ok) {
